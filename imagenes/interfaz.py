...
def seleccionar_archivo():
    ruta = filedialog.askopenfilename(filetypes=[("Archivos BMP", "*.bmp")])
    if ruta:
...
def iniciar_proceso(modo):
    ...
    if not ruta_in:
        messagebox.showwarning("Atención", "Por favor selecciona una imagen.")
        return
    ...
    if modo == 'cifrar':
        ruta_out = ruta_in.replace('.bmp','_c.bmp')
    else:
        ruta_out = ruta_in.replace('.bmp','_d.bmp')

    exito, mensaje = imagen.procesar_imagen(ruta_in, ruta_out, desplazamiento, modo)
...
ventana = tk.Tk()
ventana.title("Cifrador de Imágenes BMP")
ventana.geometry("450x200")

tk.Label(ventana, text="Selecciona la imagen BMP:").pack(pady=5)
...
tk.Button(frame_botones, text="Cifrar Imagen", bg="lightblue", width=15, 
          command=lambda: iniciar_proceso("cifrar")).pack(side=tk.LEFT, padx=10)

tk.Button(frame_botones, text="Descifrar Imagen", bg="lightgreen", width=15, 
